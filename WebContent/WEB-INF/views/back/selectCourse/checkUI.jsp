...
<nav class="breadcrumb">
	...
	<span class="c-gray en">&gt;</span>报告批阅  <span style="margin-left:120px;font-size:20px;color:red;">${student.userId}-${student.name}</span>
</nav>
...
<!-- 					    <input type="radio" id="yxdl" name="type" value="1">
					    <label for="radio-2">余弦定理</label>&nbsp; -->
					    <input type="radio" id="bjjl" name="type" value="0"  checked>
					    <label for="radio-1">编辑距离</label>&nbsp;
					    <input type="radio" id="mhpp" name="type" value="2">
					    <label for="radio-2">模糊匹配</label>&nbsp;&nbsp;
						<a href="javascript:void(0);" idVal="${experiment.id}" class="tablelink cnki" title="查重">
							<i class="icon Hui-iconfont" style="color: #000;"><font color="red">&#xe6e1;查重</font></i>
						</a>
				</div>
		</div>
	</form>
	<form action="${staticPath }/back/report/jjReport" class="form form-horizontal"  target="result_win" id="scoreForm" method="POST">
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3">评阅:</label>
			<div class="formControls col-xs-4 col-sm-6">
		...
		<%-- <div class="row cl">
			...
			<div class="formControls col-xs-4 col-sm-6">
				<input type="text" class="input-text"  placeholder="请输入分数" name="score" id ="score"/>
			</div>
		</div> --%>
		<br/>
		<input type="hidden" name="id" value="${id }" id="id">
		<input type="hidden" name="judges" id="judges" value="${judges }" />
		<div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-3">
