...
<body>
	...
	<span class="c-gray en">&gt;</span> 院系信息
	...
	<div class="page-container">
		<div class="cl pd-5 bg-1 bk-gray mt-20">
			<span class="l"> <shiro:hasPermission
					name="/back/department/departmentAdd">
					<a class="btn btn-primary radius" data-title="添加" id="add"
						href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加院系</a>
				</shiro:hasPermission> <shiro:hasPermission name="/back/department/departmentDeleteIDs">
		...
		<div class="mt_right" style="float: right;">
			<form action="${staticPath}/back/department/manager" id="pageForm"
				...
				<label class="mt_label">编码：</label> <input name="code"
					...
					placeholder="请输入..." /> <label class="mt_label">院系名称：</label> <input
					name="name" value="${department.name}" class="mt_select mt_input"
					type="text" placeholder="请输入关键字..." />
					<label class="mt_label">学校名称：</label> <input
					name="schoolName" value="${schoolName}" class="mt_select mt_input"
					type="text" placeholder="请输入关键字..." />
		...
		<div class="mt-20">
			<table
				...
				<tbody>
					<c:if
						test="${departmentList == null or departmentList.size() == 0 }">
						<tr>
							<td colspan="4">没有记录</td>
						</tr>
					</c:if>
					<c:if
						test="${departmentList != null and departmentList.size() > 0 }">
						<c:forEach items="${departmentList }" var="department">
							<tr class="text-c">
								<td><input type="checkbox" value="${department.id }"
									class="ids" name=""></td>
								<td>${department.code }</td>
								<td>${department.name}</td>
								<td>${department.school.name}</td>
								<td class="f-14 td-manage"><shiro:hasPermission
										name="/back/department/departmentUpdate">
										<a href="javascript:void(0);" idVal="${department.encryptId}"
											class="tablelink departmentUpdate"><i class="icon Hui-iconfont" style="color: #000;">&#xe6df;</i></a>
									</shiro:hasPermission> <shiro:hasPermission name="/back/department/departmentDelete">
										<a href="javascript:void(0);" idVal="${department.encryptId}"
											class="tablelink departmentDelete"><i class="icon Hui-iconfont" style="color: #000;">&#xe609;</i></a>
	...
	<script type="text/javascript">
		...
		/* 查看 */
		function departmentDetail(title, url, id) {
			index = layer.open({
				type : 2,
				title : title,
				content : url + "?id=" + id
