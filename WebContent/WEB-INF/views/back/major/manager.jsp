...
<body>
	<nav class="breadcrumb">
		...
		<span class="c-gray en">&gt;</span> 专业信息
	...
	<div class="page-container">
		<div class="cl pd-5 bg-1 bk-gray mt-20">
			<span class="l"> <shiro:hasPermission
					name="/back/major/majorAdd">
					<a class="btn btn-primary radius" data-title="添加" id="add"
						href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加专业</a>
				</shiro:hasPermission> <shiro:hasPermission name="/back/major/majorDeleteIDs">
		...
		<div class="mt_right" style="float: right;">
			<form action="${staticPath}/back/major/manager" id="pageForm"
				style="float: right;" method="post">
				<label class="mt_label">院系名称：</label> <input name="departmentName"
					value="${departmentName}" class="mt_select mt_input" type="text"
					placeholder="请输入..." /> <label class="mt_label">专业编码：</label> <input name="code"
					value="${major.code}" class="mt_select mt_input" type="text"
					placeholder="请输入..." /><label class="mt_label">专业名称：</label> <input
					name="name" value="${major.name}" class="mt_select mt_input"
		...
		<div class="mt-20">
			<table
				...
				<tbody>
					<c:if
						test="${majorList == null or majorList.size() == 0 }">
						<tr>
							<td colspan="5">没有记录</td>
						</tr>
					</c:if>
					<c:if
						test="${majorList != null and majorList.size() > 0 }">
						<c:forEach items="${majorList }" var="major">
							<tr class="text-c">
								<td><input type="checkbox" value="${major.id }"
									class="ids" name=""></td>
								<td>${major.department.name}</td>
								<td>${major.code }</td>
								<td>${major.name}</td>
								<td class="f-14 td-manage"><shiro:hasPermission
										name="/back/major/majorUpdate">
										<a href="javascript:void(0);" idVal="${major.encryptId}"
											class="tablelink majorUpdate"><i class="icon Hui-iconfont" style="color: #000;">&#xe6df;</i></a>
									</shiro:hasPermission> <shiro:hasPermission name="/back/major/majorDelete">
										<a href="javascript:void(0);" idVal="${major.encryptId}"
											class="tablelink majorDelete"><i class="icon Hui-iconfont" style="color: #000;">&#xe609;</i></a>
	...
	<script type="text/javascript">
		function jumpUrl() {
			top.layer.closeAll();
			window.location.href = "${staticPath}/back/major/manager";
		}
		var index;
		$(function() {
			$("#add").click(function() {
				top.layer.open({
					...
					content : rootPath + "/back/major/majorAddUI",btn: ['保存','取消'],
			 ...
			 function subAddForm(addFormData){
					...
					$.ajax({  
						type:'post',
						url:rootPath+"/back/major/majorAdd",
						data:addFormData,  
					    cache:false,
					    dataType:'json', 
								...
								top.layer
										.confirm(
												...
												function() {
													...
													$
															.ajax({
																url : "${staticPath }/back/major/majorDeleteIDs",
																type : "post",
																data : "ids="
																		+ ids,
																dataType : "json",
		...
		/* 查看 */
		function majorDetail(title, url, id) {
			index = layer.open({
				type : 2,
				title : title,
				content : url + "?id=" + id
		...
		function subEditForm(addFormData) {
			...
			$.ajax({
				type : 'post',
				url : rootPath + "/back/major/majorUpdate",
				data : addFormData,
				cache : false,
				dataType : 'json',
				success : function(data) {
		...
		$(".majorDelete").click(function() {
			...
			//按钮
				...
				$.ajax({
					type : 'get',
					url : rootPath + "/back/major/majorDelete/" + id,
					cache : false,
					dataType : 'json',
					success : function(data) {
						if (data.success == true) {
